...
#Labelling the plot
title(main="Title", sub="Subtitle", xlab="x-axis label", ylab="y-axis label")
...
#Adding text
f + annotate("text", x=x_small, y=y_small, label=paste("Put some text at position", 1:5))
#Text (with background label)
f + annotate("label", x=x_small, y=y_small, label=paste("Put some text at position", 1:5))

#Adding a grid and controlling limits (done by default)
f + coord_cartesian(xlim=c(0, 1), ylim=c(0, 1))
#Adding polar coordinates instead (e.g. to transform a bar chart into a pie chart)
f + coord_polar()

#Labelling the plot
f + labs(x="x-axis label", y="y-axis label", title="Title", subtitle="Subtitle", caption="caption")
...
ggsave(filename="my_graphic.png")